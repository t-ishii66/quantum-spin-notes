...
つまり、**測定確率はブロッホ球上の角度 $\Theta$ だけで決まる**。この公式は [NOTE1.md](NOTE1.md) で導入した Born 則 $P(\phi) = \vert \langle\phi\vert \psi\rangle\vert ^2$ をブロッホ球の幾何学で書き直したものである。ブロッホ球の上では、2つの状態がどれだけ近いか（角度が小さいか）で測定結果の確率が決まる。
