...
の形になるのかを、一つの流れで説明する。ここで $\theta$ は軸 $\mathbf{n}$ まわりの**回転角**であり、ブロッホ球（スピン状態を球面上の点として表す方法。[NOTE3.md](NOTE3.md) で解説する）の極角とは異なる量である。
...
ここまでの議論で、位置と運動量を持つ粒子の回転生成子は軌道角運動量 $\hat{\mathbf{L}}$ であることが分かった。回転の生成子を一般に $\mathbf{J}$ と書くと、結果をまとめることができる。

**微小回転による演算子の変化**——先に出てきた $\delta\hat{A}$ の式に $G_{\mathbf{n}} = \mathbf{n}\cdot\mathbf{J}$ を代入すると
...
で与えられる。状態を $U$ で回すか、演算子を $U^{\dagger}\hat{A}\,U$ で逆に回すかは、同じ物理を反対側から見ているだけである。
