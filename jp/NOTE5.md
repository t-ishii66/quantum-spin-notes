...
これは机上の議論ではない。実験が決着をつけている。

1972年の Freedman と Clauser に始まり、1982年の Aspect らの実験、そして2015年の「抜け穴なし（loophole-free）」実験に至るまで、光子の偏光や原子のスピンを使った数多くの実験で、CHSH 不等式の古典上限 $\vert S\vert \leq 2$ は**繰り返し破られている**。特に、2つの測定装置の間を光が届くよりも短い時間で測定方向を切り替える実験では、一方の結果が他方に光速以下の信号で伝わる可能性すら排除されている。

自然は、私たちの素朴な直観——測定前から結果は決まっているはずだ、離れた場所の操作が瞬時に影響するはずがない——を、明確に裏切った。量子力学の予測 $\vert S\vert = 2\sqrt{2}$ のほうが正しいのである。

2022年のノーベル物理学賞は、まさにこのベルの不等式の実験的検証に対して、Aspect、Clauser、Zeilinger の3名に授与された。半世紀にわたる実験の積み重ねが、量子もつれが単なる理論上の奇妙さではなく、自然の根本的な性質であることを確立したのである。
